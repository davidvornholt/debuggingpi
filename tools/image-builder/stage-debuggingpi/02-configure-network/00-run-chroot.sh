...
install -m 600 files/debuggingpi-ap.nmconnection "$NM_DIR/debuggingpi-ap.nmconnection"

# USB network profile for Pi Zero W
install -m 600 files/usb-zero.nmconnection "$NM_DIR/usb-zero.nmconnection"

echo "[debuggingpi] NetworkManager profiles installed"

# IP forwarding
install -m 644 files/debuggingpi-forwarding.conf /etc/sysctl.d/90-debuggingpi.conf
